from lib2.utils import *
from cvxopt import matrix
from lib2.l1 import l1
import numpy as np

"""
Package obtained from: https://github.com/LeeDoYup/RobustSTL
Based on paper: RobustSTL: A Robust Seasonal-Trend Decomposition Algorithm for Long Time Series
Slightly adjusted to not get into a infinite loop, if there is no convergence
"""
...
def trend_extraction(sample, season_len, reg1=10., reg2=0.5):
    ...
    try:
        ...
        delta_trends = np.zeros(sample_len-1)
...
def seasonality_extraction(sample, season_len=10, K=2, H=5, ds1=50., ds2=1.):
    ...
    def get_season_value(idx):
        ...
        # Adjustment for if all weights are 0
        if np.sum(weights) == 0:
            season_value = 0
        else:
