...
class Clusters:
    ...
    def k_medoids(self, num_clusters):
        ...
        # create list with cluster labels
    ...
    def GAP(self, method, measure, nrefs=250, maxClusters=10):
        """
        ...
        for gap_index, k in enumerate(range(1, maxClusters)):
            ...
            for i in range(nrefs):
                ...
                for j in range(len(self.distances.ts_matrix)):
                    arparams = np.array([0.9, -0.5])
                    maparams = np.array([0.25, 0])
        ...
        # if method == 'k_medoids':
        if np.nonzero(np.array(gaps[0:(maxClusters-2)]) >= np.array((gaps[1:(maxClusters-1)] - s_ks[1:(maxClusters-1)])))[0].tolist() and k>1:
